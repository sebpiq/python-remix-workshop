...
def _peakdetect(y_axis, x_axis = None, lookahead = 300, delta=0):
    """
...
def peakdetect(*args, **kwargs):
    """
    customized to smooth out peaks
    """
    frame_rate = 44100.0
    release_time = kwargs.pop('release', 0.02)
    second_pass = kwargs.pop('second_pass', False)
    y = args[0]
    frame_count = len(y)
    _max, _min = _peakdetect(*args, **kwargs)

    # concatenate maxs and mins and reorder them
    if _max and _min:
        peaks = numpy.vstack([_max, _min])
    elif _max or _min: peaks = numpy.array(_min or _max)
    else: return numpy.array([])
    peaks = peaks[peaks[:, 0].argsort()]

    # second detection to smooth out results
    if second_pass:
        smoothed = numpy.interp(numpy.arange(0, frame_count) / frame_rate, peaks[:, 0], peaks[:, 1])
        peaks, _min = _peakdetect(smoothed, numpy.arange(0, frame_count) / frame_rate, **kwargs)
    ...
    peaks_bis = []
    for peak in peaks:
        if last_peak is not None and (peak[0] - last_peak[0]) < release_time:
            continue
        last_peak = peak
        peaks_bis.append(peak)
    return numpy.array(peaks_bis)
